...
public class UsersController {
    ...
    private TableView<Account> createTable() {
        ...
        TableColumn<Account,String> userNameColumn = new TableColumn<>("Никнейм");
        userNameColumn.setCellValueFactory(param -> param.getValue().userName);
        userNameColumn.setPrefWidth(150);
        userNameColumn.setMinWidth(100);
        userNameColumn.setSortable(false);

        TableColumn<Account,String> emailColumn = new TableColumn<>("E-Mail");
    ...
    void initialize() {
        ...
        changeRoleButton.setOnAction(actionEvent -> {
            if (table.getSelectionModel().getSelectedIndex() != -1) {
                ...
                alert.setTitle("Предупреждение");
                alert.setHeaderText("Ошибка выделения строки!");
                alert.setContentText("Выделите строку в таблице аккаунтов, чтобы поменять роль!");
                alert.showAndWait();
            }
        });
    }
